...
public class BushBlockRenderer implements ISimpleBlockRenderingHandler {
    ...
    private void renderBranches(IBlockAccess world, int x, int y, int z, Block block, RenderBlocks renderer, BushBlock bushBlock) {
        double coreMin = 0.5 - HALF_WIDTH * 0.0625;
        double coreMax = 0.5 + HALF_WIDTH * 0.0625;
        ...
        for(ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS) {
            ...
            switch (direction) {
                case EAST:
                    renderCrossedBranch(world, x, y, z, block, bushBlock.getBranchIcon(),
                        min, coreMin, coreMin, max, coreMax, coreMax, direction);
                    break;

                case UP:
                    renderCrossedBranch(world, x, y, z, block, bushBlock.getBranchIcon(),
                        coreMin, min, coreMin, coreMax, max, coreMax, direction);
                    break;

                case SOUTH:
                    renderCrossedBranch(world, x, y, z, block, bushBlock.getBranchIcon(),
                        coreMin, coreMin, min, coreMax, coreMax, max, direction);
                    break;
    ...
    private void renderBranchesCuboid(IBlockAccess world, int x, int y, int z, Block block, RenderBlocks renderer, BushBlock bushBlock) {
        double coreMin = 0.5 - HALF_WIDTH * 0.0625;
        double coreMax = 0.5 + HALF_WIDTH * 0.0625;
        ...
        for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if (!world.isSideSolid(x + direction.offsetX, y + direction.offsetY, z + direction.offsetZ, direction.getOpposite(), false)
                && !(world.getBlock(x + direction.offsetX, y + direction.offsetY, z + direction.offsetZ) instanceof BushBlock))
                continue;
    ...
    private void renderInventoryCrossedBranch(
        IIcon icon,
        ...
        addDoubleSidedQuad(tessellator,
    ...
    private void renderCrossedBranch(
        IBlockAccess world,
        int x, int y, int z,
        Block block,
        IIcon icon,
        ...
        switch (direction) {
            ...
            case SOUTH:
                ...
                addDoubleSidedQuad(tessellator,
                    x + minX, y + minY, z + minZ, uMin, vMax,
                    x + maxX, y + maxY, z + minZ, uMin, vMin,
                    x + maxX, y + maxY, z + maxZ, uMax, vMin,
                    x + minX, y + minY, z + maxZ, uMax, vMax);
                addDoubleSidedQuad(tessellator,
                    x + maxX, y + minY, z + minZ, uMin, vMax,
                    x + minX, y + maxY, z + minZ, uMin, vMin,
                    x + minX, y + maxY, z + maxZ, uMax, vMin,
                    x + maxX, y + minY, z + maxZ, uMax, vMax);
                break;
    ...
    private boolean isBlockConnected(IBlockAccess world, int x, int y, int z, ForgeDirection direction) {
        return world.isSideSolid(x + direction.offsetX, y + direction.offsetY, z + direction.offsetZ, direction.getOpposite(), false)
            || (world.getBlock(x + direction.offsetX, y + direction.offsetY, z + direction.offsetZ) instanceof BushBlock);
    }
}
